...
state Animating
	...
	function OrgasmEffectSLSO(bool Force = true)
		...
		If Force != true && SeparateOrgasms
			...
			if SLSO_DoOrgasm_Conditions(Force) < 0
				return
			endIf
		endIf
...
int function GetEnjoyment()
	...
	elseif !IsSkilled
		...
		endIf
	endIf
	int SLSO_Enjoyment = Enjoyment - BaseEnjoyment
	;Log("SLSO_GetEnjoyment: SLSO_Enjoyment["+SLSO_Enjoyment+"] / Enjoyment["+Enjoyment+"] / BaseEnjoyment["+BaseEnjoyment+"]")
	if SLSO_Enjoyment > 0
...
function SLSO_DoOrgasm_Multiorgasm()
	...
	if (Utility.RandomInt(0, 100) > (JsonUtil.GetIntValue(File, "sl_multiorgasmchance") + ((OwnSkills[Stats.kLewd] * JsonUtil.GetIntValue(File, "sl_multiorgasmchance_curve")) as int) - 10 * Orgasms)) || BaseSex != 1
		...
		if SeparateOrgasms
			if BaseEnjoyment < 50 && BaseEnjoyment > -50
			...
			if JsonUtil.GetIntValue(File, "game_enabled") == 1
				if GetOrgasmCount() == Thread.Get_minimum_aggressor_orgasm_Count()
					if Utility.RandomInt(0, 100) < JsonUtil.GetIntValue(File, "condition_chance_minimum_aggressor_orgasm_increase")
						...
						Log("Lead actor - " + ActorName + " increased required orgasms to: " + Thread.Get_minimum_aggressor_orgasm_Count())
					endif
				endif
			endif
		endif
